...
class NetworkUtil {
  ...
  Map<String, dynamic>? getAuthorizationHeader() {
    ...
    var sharedPref = SharedPrefUtil();
    bool isLoggedIn = sharedPref.getBool(SharedPrefUtil.keyIsLoggedIn);
    if (isLoggedIn) {
      var token = sharedPref.getString(SharedPrefUtil.keyUserToken);
      if (token.isNotEmpty) {
  ...
  ErrorEntity _handleError(error) {
    ...
    if (error is DioException) {
      ...
      return ErrorEntity(code: -1, message: errorMessage);
    }
  }
}
...
class ErrorEntity implements Exception {
  ...
  ErrorEntity({required this.code, required this.message, this.data});
